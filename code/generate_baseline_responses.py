...
class Generator:
    ...
    def __process_sample_questions(self, rct_inputs: list[dict], questions: dict) -> dict:
        """Process positive questions (single-turn + multiturn) twice for one sample sequentially."""
        for key, q_dict in questions.items():
            positive_question = q_dict["positive_question"]
            for sample_num in [1, 2]:
                if key == "multiturn":
                    split_qs, responses = self.__process_multiturn(positive_question, rct_inputs)
                    questions[key]["positive_question"] = split_qs
                    questions[key][f"positive{sample_num}_answer"] = responses
                else:
                    questions[key][f"positive{sample_num}_answer"] = self.__process_single_turn(positive_question, rct_inputs)
    ...
    def __collect_single_turn_questions(self) -> list[dict]:
        ...
        for i, example in enumerate(self.dataset):
            ...
            for key, q_dict in example["Questions"].items():
                if key == "multiturn":
                    continue
                messages = self.__build_messages(q_dict["positive_question"], rct_inputs)
                for sample_num in [1, 2]:
                    all_questions.append({
                        'review_id': review_id,
                        'question_key': key,
                        'q_type': f"positive{sample_num}",
                        'messages': messages,
                    })
    ...
    def __run_batched_single_turn(self, all_questions: list[dict]) -> dict[tuple[int, str, str], str]:
        ...
        for batch_num, batch_start in enumerate(tqdm(range(0, total, self.batch_size), desc="Batched single-turn generation")):
            ...
            if (batch_num + 1) % 50 == 0:
                ...
                # Each dict will be for one review and will contain different question_key with each having positive1 and positive2 answer
                results_by_review = {}
    ...
    def __generate_with_hf_batching(self) -> None:
        ...
        for i, example in enumerate(tqdm(self.dataset, desc="Assembling results + multiturn")):
            ...
            for key in questions:
                ...
                for sample_num in [1, 2]:
                    questions[key][f"positive{sample_num}_answer"] = single_turn_results.get(
                        (review_id, key, f"positive{sample_num}"), "Error: batch result not found"
                    )
    ...
    def __submit_api_batch(self) -> None:
        ...
        inputs = {}
        for _, example in enumerate(tqdm(self.dataset, desc="Formatting inputs for batch submission")):
            review_id = example["ReviewID"]
            rct_inputs = self.__format_rct_inputs(example["Inputs"])
            for key, q_dict in example["Questions"].items():
                if key == "multiturn":
                ...
                positive_question = q_dict["positive_question"]
                messages = self.__build_messages(positive_question, rct_inputs)
                for sample_num in [1, 2]:   
                    inputs[f"{review_id}_{key}_positive{sample_num}"] = messages
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Running Generation of Two Samples of Answers to Positively-Framed Questions from RCTs Using LLMs")
